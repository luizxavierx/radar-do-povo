...
final class RadarServiceProvider extends ServiceProvider
{
    public function register(): void
    {
        ...
        $this->app->bind(CacheRepository::class, function ($app): CacheRepository {
            ...
            $fallbackStore = config('cache.stores.file') !== null ? 'file' : 'array';
            $redisClient = (string) config('database.redis.client', 'phpredis');

            if ($store === 'redis') {
                if ($redisClient === 'phpredis' && ! class_exists(\Redis::class)) {
                    $store = $fallbackStore;
                }

                if ($redisClient === 'predis' && ! class_exists(\Predis\Client::class)) {
                    $store = $fallbackStore;
                }
            }
            ...
            try {
                ...
                return $cache->store($fallbackStore);
            }
        });
    }

...
final class GastoService
{
    ...
    ): array
    {
        ...
        $cacheKey = CacheKeyFactory::gastoResumoViagens($filtro, $includePagamentos, $includeTrechos);
        $ttl = (int) config('radar.query_cache_ttl_seconds', 300);

        return $this->cache->remember(
            $cacheKey,
            $ttl,
            fn (): array => $this->repository->summaryViagens($filtro, $includePagamentos, $includeTrechos),
        );
    }
    ...
    public function listViagens(array $filtro, int $limit, int $offset, bool $includeTotal = true): array
    {
        ...
        $cacheKey = CacheKeyFactory::gastoViagensPainel($filtro, $limit, $offset, $includeTotal);
        $ttl = (int) config('radar.query_cache_ttl_seconds', 300);

        return $this->cache->remember(
            $cacheKey,
            $ttl,
            fn (): array => $this->repository->listViagens($filtro, $limit, $offset, $includeTotal),
        );
    }
    ...
    ): array
    {
        ...
        $cacheKey = CacheKeyFactory::gastoTopViajantes($filtro, $limit, $offset, $includeTotal, $nodeFields);
        $ttl = (int) config('radar.query_cache_ttl_seconds', 300);

        return $this->cache->remember(
            $cacheKey,
            $ttl,
            fn (): array => $this->repository->topViajantes($filtro, $limit, $offset, $includeTotal, $nodeFields),
        );
    }
    ...
    ): array
    {
        ...
        $cacheKey = CacheKeyFactory::gastoTopGastadoresViagens($filtro, $limit, $offset, $includeTotal, $nodeFields);
        $ttl = (int) config('radar.query_cache_ttl_seconds', 300);

        return $this->cache->remember(
            $cacheKey,
            $ttl,
            fn (): array => $this->repository->topGastadoresViagens($filtro, $limit, $offset, $includeTotal, $nodeFields),
        );
    }
    ...
    ): array
    {
        ...
        $cacheKey = CacheKeyFactory::gastoTopOrgaosSuperioresViagens($filtro, $limit, $offset, $includeTotal, $nodeFields);
        $ttl = (int) config('radar.query_cache_ttl_seconds', 300);

        return $this->cache->remember(
            $cacheKey,
            $ttl,
            fn (): array => $this->repository->topOrgaosSuperioresViagens($filtro, $limit, $offset, $includeTotal, $nodeFields),
        );
    }
    ...
    ): array
    {
        ...
        $cacheKey = CacheKeyFactory::gastoTopOrgaosSolicitantesViagens($filtro, $limit, $offset, $includeTotal, $nodeFields);
        $ttl = (int) config('radar.query_cache_ttl_seconds', 300);

        return $this->cache->remember(
            $cacheKey,
            $ttl,
            fn (): array => $this->repository->topOrgaosSolicitantesViagens($filtro, $limit, $offset, $includeTotal, $nodeFields),
        );
    }

...
final class ViagemQuery
{
     ...
     * @return array{nodes: array<int,array<string,mixed>>, total: int, limit: int, offset: int}
     */
    public function topViajantes(mixed $_, array $args, mixed $context, ResolveInfo $resolveInfo): array
    {
        $filtro = $this->normalizeRankingFiltro($args['filtro'] ?? []);
        if ($filtro['apenasParlamentares'] === null) {
            $filtro['apenasParlamentares'] = false;
        ...
        $pagination = $this->normalizePagination($args['pagination'] ?? []);
        $selection = $this->connectionSelection($resolveInfo);
        $result = $this->gastoService->topViajantes(
            $filtro,
            $pagination['limit'],
            $pagination['offset'],
            $selection['includeTotal'],
            $selection['nodeFields'],
        );
     ...
     * @return array{nodes: array<int,array<string,mixed>>, total: int, limit: int, offset: int}
     */
    public function topOrgaosSuperioresViagens(mixed $_, array $args, mixed $context, ResolveInfo $resolveInfo): array
    {
        $filtro = $this->normalizeRankingFiltro($args['filtro'] ?? []);
        if ($filtro['apenasParlamentares'] === null) {
            $filtro['apenasParlamentares'] = false;
        ...
        $pagination = $this->normalizePagination($args['pagination'] ?? []);
        $selection = $this->connectionSelection($resolveInfo);
        $result = $this->gastoService->topOrgaosSuperioresViagens(
            $filtro,
            $pagination['limit'],
            $pagination['offset'],
            $selection['includeTotal'],
     ...
     * @return array{nodes: array<int,array<string,mixed>>, total: int, limit: int, offset: int}
     */
    public function topOrgaosSolicitantesViagens(mixed $_, array $args, mixed $context, ResolveInfo $resolveInfo): array
    {
        $filtro = $this->normalizeRankingFiltro($args['filtro'] ?? []);
        if ($filtro['apenasParlamentares'] === null) {
            $filtro['apenasParlamentares'] = false;
        ...
        $pagination = $this->normalizePagination($args['pagination'] ?? []);
        $selection = $this->connectionSelection($resolveInfo);
        $result = $this->gastoService->topOrgaosSolicitantesViagens(
            $filtro,
            $pagination['limit'],
            $pagination['offset'],
            $selection['includeTotal'],
     ...
     * @return array{nodes: array<int,array<string,mixed>>, total: int, limit: int, offset: int}
     */
    public function topGastadoresViagens(mixed $_, array $args, mixed $context, ResolveInfo $resolveInfo): array
    {
        $filtro = $this->normalizeRankingFiltro($args['filtro'] ?? []);
        if ($filtro['apenasParlamentares'] === null) {
            $filtro['apenasParlamentares'] = false;
        ...
        $pagination = $this->normalizePagination($args['pagination'] ?? []);
        $selection = $this->connectionSelection($resolveInfo);
        $result = $this->gastoService->topGastadoresViagens(
            $filtro,
            $pagination['limit'],
            $pagination['offset'],
            $selection['includeTotal'],
    ...
    /**
     * @return array{includeTotal: bool, nodeFields: array<int,string>}
     */
    private function connectionSelection(ResolveInfo $resolveInfo): array
    {
        $selection = $resolveInfo->getFieldSelection(2);
        $nodeSelection = $selection['nodes'] ?? [];
        $nodeFields = is_array($nodeSelection)
            ? array_values(array_filter(array_keys($nodeSelection), static fn (mixed $field): bool => is_string($field)))
            : [];

        return [
            'includeTotal' => array_key_exists('total', $selection),
